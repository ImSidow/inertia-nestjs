import {
    Controller,
    Get,
    MiddlewareConsumer,
    Module,
    NestModule,
} from '@nestjs/common';
import { Test } from '@nestjs/testing';
import type { INestApplication } from '@nestjs/common';
import request from 'supertest';
import { HandleInertiaRequests, Inertia, InertiaModule } from '../src';

@Controller()
class TestController {
    @Get('/users')
    @Inertia('Users/Index')
    users() {
        return {
            users: [
                { id: 1, name: 'Alice' },
                { id: 2, name: 'Bob' },
            ],
        };
    }
}

@Module({
    imports: [
        InertiaModule.forRoot({
            rootView: 'app',
            version: '1.0.0',
        }),
    ],
    controllers: [TestController],
})
class TestAppModule implements NestModule {
    configure(consumer: MiddlewareConsumer) {
        consumer.apply(HandleInertiaRequests).forRoutes('*');
    }
}

describe('@Inertia() decorator (e2e)', () => {
    let app: INestApplication;

    beforeAll(async () => {
        const moduleRef = await Test.createTestingModule({
            imports: [TestAppModule],
        }).compile();

        app = moduleRef.createNestApplication();
        await app.init();
    });

    afterAll(async () => {
        await app.close();
    });

    it('returns an Inertia JSON page for X-Inertia requests', async () => {
        const res = await request(app.getHttpServer())
            .get('/users')
            .set('X-Inertia', 'true')
            .set('X-Inertia-Version', '1.0.0')
            .expect(200);

        expect(res.headers['x-inertia']).toBe('true');
        expect(res.body.component).toBe('Users/Index');
        expect(res.body.url).toBe('/users');
        expect(res.body.version).toBe('1.0.0');
        expect(res.body.props.users).toEqual([
            { id: 1, name: 'Alice' },
            { id: 2, name: 'Bob' },
        ]);
    });
});
